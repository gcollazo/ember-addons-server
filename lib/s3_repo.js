...
var S3Repo = function(options) {
  ...
  this.pagesFilename = this.options.pagesFilename;
  this.addonFilename = this.options.addonFilename;
  this.feedFilename = this.options.feedFilename;
 ...
 * of arrays specified by the slices arg
 *
...
S3Repo.prototype._prepareAddonData = function(addonData) {
  var filtered = addonData.filter(function(item) {
    if (item.name) {
      return item;
    }
  });
  return JSON.stringify(filtered.map(function(item) {
    var demoURL;

    if (item.latest['ember-addon']) {
      demoURL = item.latest['ember-addon'].demoURL;
    } else {
      demoURL = null;
    }
    ...
    return  {
      ...
      emberObserver: item.emberObserver,
      github: item.github,
      name: item.name,
      time: item.time
    };
...
S3Repo.prototype.saveAddonPages = function(addonData) {
  ...
  var promises = pages.map(function(page, index) {
    ...
    pageFilenames
      .push('https://' + self.bucket + '.s3.amazonaws.com/' + filename);
...
S3Repo.prototype.saveAddonFeed = function(feedXML) {
  return this._putString({
    data:feedXML,
